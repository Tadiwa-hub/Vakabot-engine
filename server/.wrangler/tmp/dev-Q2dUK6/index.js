...
// .wrangler/tmp/bundle-yDSQQf/checked-fetch.js
...
// .wrangler/tmp/bundle-yDSQQf/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-yDSQQf/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
var autoReplies = sqliteTable("auto_replies", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull().references(() => users.id),
...
var EvolutionClient = class {
  ...
  async getConnectData(instanceName) {
  ...
  async sendMessage(instanceName, remoteJid, text2) {
...
var app = new Hono2();
app.use("*", cors());
var getTools = /* @__PURE__ */ __name((env) => {
  const db = getDb(env);
...
app.get("/api/instance/:userId", async (c) => {
  ...
  const [instance] = await db.select().from(instances).where(eq(instances.userId, userId)).orderBy(desc(instances.updatedAt)).limit(1);
  if (instance && instance.status !== "CONNECTED") {
    try {
      ...
      if (newStatus !== instance.status || newQr !== instance.qrcode) {
        await db.update(instances).set({ status: newStatus, qrcode: newQr, updatedAt: (/* @__PURE__ */ new Date()).toISOString() }).where(eq(instances.id, instance.id));
...
app.post("/api/webhook/config", async (c) => {
  ...
  await db.update(instances).set({ webhookUrl, updatedAt: (/* @__PURE__ */ new Date()).toISOString() }).where(eq(instances.id, instance.id));
...
app.post("/api/instance/:userId/disconnect", async (c) => {
  ...
  await db.update(instances).set({ status: "DISCONNECTED", qrcode: null, isActive: false, updatedAt: (/* @__PURE__ */ new Date()).toISOString() }).where(eq(instances.id, instance.id));
...
app.post("/api/instance/:userId/toggle", async (c) => {
  ...
  await db.update(instances).set({ isActive: newStatus, updatedAt: (/* @__PURE__ */ new Date()).toISOString() }).where(eq(instances.id, instance.id));
...
app.post("*", async (c, next) => {
  console.log(`[CATCH ALL POST] Path: ${c.req.path}`);
  try {
    const rawText = await c.req.raw.clone().text();
    console.log(`[CATCH ALL BODY]:`, rawText.substring(0, 500));
  } catch (e) {
  }
  await next();
});
app.post("/webhook/evolution", async (c) => {
  try {
    ...
    const message = data.message?.conversation || data.message?.extendedTextMessage?.text || data.message?.imageMessage?.caption || "";
    const remoteJid = data.key?.remoteJid;
    const fromMe = data.key?.fromMe;
    console.log(`[Webhook] Inbound: "${message}" from ${remoteJid} (fromMe: ${fromMe})`);
    if (!message || !remoteJid || fromMe) return c.text("OK");
    ...
    const match2 = rulesList.find(
      (r) => r.isActive && message.toLowerCase().trim().includes(r.keyword.toLowerCase().trim())
    );
    if (match2) {
      console.log(`[Webhook] MATCH! Sending: ${match2.replyText}`);
      await evolution.sendMessage(instanceName, remoteJid, match2.replyText);
    } else {
